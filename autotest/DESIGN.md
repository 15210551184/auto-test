...
候选不止一个时按三层证据依次纠偏（`guess_list_api()`）：

1. **内容比对**——表格第一行的样本值（`scan_table()` 顺手拿到的
   `sample_row`）有没有出现在候选接口的响应体里，出现最多的那个胜出。
   只用长度 ≥4 的样本值参与比对（"启用"/"否"这类太通用，随便什么响应
   都可能碰巧包含，反而误判；日期、手机号、名称这类值够独特）。这是
   最硬的证据："响应里真有页面上看到的这行数据"。
2. **业务词比对**——没有内容证据时，退到比较页面自己的地址（如
   `/web/country/list`）跟候选接口路径有没有共同的业务词（如
   `country`）。
3. 都没有信号（页面地址提取不出英文业务词，样本值也没在任何候选响应体
   里出现过）就退回选"最后一条"，跟原来一样。

第 1、2 层是后补的：原来直接选"最后一条命中关键词的"，真实事故是某个
全局小组件（通知公告、未读消息角标）也打带 `list` 字样的接口，还是
定时轮询的——扫描全程耗时几秒到几十秒，轮询几次就正好排到最后，把这个
页面自己的列表接口顶掉，生成的 `list_api` 配置执行时永远等不到匹配
响应。

**猜错了不用整页重新扫**：`list_api` 猜错是最常见的"只有一个字段不对"
场景，为了修它没必要走"重新生成用例"——那条路径连表单/表头/按钮/弹窗
一起重新识别一遍，本来就没错的部分陪跑不说，还会把手工加在这份配置里
的业务断言一起冲掉。控制台"已生成用例"的页面旁边有个「重探接口」按钮
（`scanner.redetect_list_api()` + `batch.redetect_list_api()`），只做
`scan_table()`（为了拿 `sample_row` 做内容比对）+ `guess_list_api()`，
跳过表单/按钮/分页/弹窗识别，只改配置文件里的 `list_api` 一个字段，
其余原样保留；CLI 对应 `python cli.py redetect-list-api <project> <page>`。
...
页面慢，不是接口/并发问题，是扫描这些控件、遍历这些字段确实要花更长
时间——不该为了这几个页面把全局超时都调大，拖慢"发现真正卡死页面"
的速度。项目设置里给这一页的 `pages` 条目单独加 `scan_timeout`（秒）
就行：

```yaml
pages:
- name: 计价规则配置
  url: http://.../pricing/ruleConfig
  scan_timeout: 500
```

不加就用 `SCAN_TIMEOUT_SEC` 全局默认（叠加多语言预算后的那个值）。
...
| 4 | 抽样行字段值与页面一致 | **单位错、时区错、格式错、字段本身映射错**（导出的是 ID，页面显示的是名称这种） |

**第 4 层比对哪些列，之前踩过一次坑**：`to_config()` 生成的
`export_and_verify.columns` 曾经只挑前 5 个"类型像样"的列（money/date/
phone/text）——真实事故：某页面第 7 列"加盟商"导出的是内部 ID、页面上
显示的是名称，这正是值比对该抓的问题，但排在第 5 列之后，从来没被比过，
报告上「抽样比对 N 个字段一致」看着全绿，其实这一列根本没被检查。字段值
比对是纯字符串比较，几十列也没有明显开销，现在改成把所有"类型像样"的列
都放进去比，不再截前 5 个。
